    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-display font-black text-slate-800 tracking-tight leading-none">Shift<span className="text-primary-500 italic">.Logs</span></h1>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-3">High Precision Temporal Registry</p>
                </div>
                <div className="flex items-center gap-4 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] px-6 py-3 rounded-2xl border border-slate-100/50 backdrop-blur-sm group">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
                ...
                <div className="xl:col-span-5 space-y-10">
                    <div className="card-premium p-12 overflow-hidden relative group">
                        {/* Immersive Background */}
                        <div className="absolute top-0 right-0 p-16 opacity-[0.03] group-hover:opacity-[0.06] transition-opacity duration-700 scale-[2]">
                            <Fingerprint size={100} className="text-slate-900" />
                        </div>

                        <div className="text-center space-y-10 relative z-10">
                            <div className="flex flex-col items-center gap-4">
                                <div className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border shadow-sm transition-all duration-500 ${status === 'checked-in' ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                                    {status === 'checked-in' ? 'Pulse Active' : 'Pulse Terminated'}
                                </div>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-relaxed">Identity Verified: Biometric Node Alpha</p>
                            </div>
                ...
                <div className="xl:col-span-7 h-full">
                    <div className="card-premium flex flex-col h-full overflow-hidden bg-white/50 backdrop-blur-sm border-white/50 shadow-[0_20px_50px_rgba(0,0,0,0.03)]">
                        <div className="p-8 border-b border-slate-100/50 flex items-center justify-between">
                            <div>
                                <h2 className="font-display font-black text-slate-800 tracking-tight">Temporal <span className="text-primary-500">Registry</span></h2>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Personnel Operation Logs</p>
                            </div>
                            <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-50 shadow-sm">
                                <Calendar size={16} className="text-primary-500" />
                                <span className="text-[10px] font-black text-slate-800 uppercase tracking-wider">Current Cycle</span>
                            </div>
                        </div>

                        <div className="flex-1 overflow-y-auto max-h-[600px] px-2 py-4 space-y-3 sidebar-scroll">
                            {history.length > 0 ? history.map((entry, idx) => (
                                <div key={entry._id} className="p-6 bg-white rounded-2xl border border-slate-50 shadow-sm hover:shadow-md hover:border-primary-100 transition-all group flex items-center justify-between">
                                    <div className="flex items-center gap-8">
                                        <div className="text-center min-w-[60px] p-3 bg-slate-50 rounded-2xl group-hover:bg-primary-50 transition-colors">
                                            <p className="text-2xl font-display font-black text-slate-800 tracking-tighter leading-none mb-1">
                                                {new Date(entry.date).getDate()}
                                            </p>
                                            <p className="text-[9px] font-black text-primary-500 uppercase tracking-widest leading-none">
                                                {new Date(entry.date).toLocaleString('en-US', { month: 'short' })}
                                            </p>
                                        </div>

                                        <div className="grid grid-cols-2 gap-10">
                                            <TimeMetric label="Authorization" time={entry.checkIn} color="text-emerald-500" />
                                            <TimeMetric label="Termination" time={entry.checkOut || '--:--'} color="text-slate-400" />
                                        </div>
                                    </div>

                                    <div className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] shadow-sm border ${entry.status === 'Present' ? 'text-emerald-500 bg-emerald-50 border-emerald-100' : 'text-orange-500 bg-orange-50 border-orange-100'}`}>
                                        {entry.status}
                                    </div>
                                </div>
                            )) : (
                                <div className="p-32 text-center flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-xs font-black text-slate-300 uppercase tracking-[0.4em]">Zero Telemetry Detected</p>
                                </div>
                            )}
                        </div>
                    </div>
...
const PresenceMetric = ({ label, value, unit, icon }) => (
    <div className="card-premium p-6 flex flex-col items-center justify-center text-center gap-2 group hover:scale-[1.02]">
        <div className="p-2 bg-slate-50 rounded-lg text-slate-400 group-hover:text-primary-500 group-hover:bg-primary-50 transition-colors">
            {icon}
        </div>
        <div>
