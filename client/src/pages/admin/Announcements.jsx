        ...
        <div className="min-h-screen bg-sky-50 p-4 sm:p-6 md:p-10 space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
            <AnimatePresence>
                {notification && (
                    ...
                    >
                        <div className={`px-8 py-4 rounded-2xl shadow-2xl border backdrop-blur-xl flex items-center gap-4 ${notification.type === 'error' ? 'bg-red-500 text-slate-800 border-red-400' : 'bg-emerald-500 text-slate-800 border-emerald-400'}`}>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-display font-black text-slate-800 tracking-tight leading-none uppercase">Broadcast<span className="text-sky-500">.Control</span></h1>
            ...
            <div className="grid grid-cols-1 gap-6">
                {loading ? (
                    <div className="py-20 text-center flex flex-col items-center justify-center bg-white/50 backdrop-blur-xl rounded-[2rem] border border-sky-100 shadow-2xl">
                        ...
                        >
                            <div className="space-y-4 flex-1">
                                <div className="flex items-center gap-4">
                                    <div className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${item.priority === 'high' ? 'bg-red-500/10 text-red-500 border-red-500/20' : item.priority === 'medium' ? 'bg-sky-500/10 text-sky-500 border-sky-500/20' : 'bg-slate-700 text-slate-400 border-sky-100'}`}>
                                        {item.priority} Priority
                                    </div>
                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 pt-2">
                                    <div className="w-6 h-6 rounded-lg bg-sky-50 flex items-center justify-center text-[10px] font-black text-slate-400 uppercase border border-sky-100">
                            ...
                            {isAdmin && (
                                <button
                                    ...
                                    className="p-4 bg-sky-50 text-slate-500 hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all border border-sky-100"
                                >
                                    <Trash2 size={20} />
                                </button>
                            )}
                    ...
                    <div className="py-32 text-center flex flex-col items-center justify-center bg-white/50 backdrop-blur-xl rounded-[2rem] border border-sky-100 shadow-2xl">
                        <div className="w-20 h-20 bg-sky-50 rounded-[2rem] flex items-center justify-center mb-6 text-slate-700 shadow-inner">
                            <Megaphone size={40} />
                        </div>
                        <p className="text-xs font-black text-slate-500 uppercase tracking-[0.4em]">Static detected. Zero active broadcasts.</p>
                    </div>
                )}
            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                                <div className="flex items-center justify-between border-b border-sky-100 pb-6">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setShowModal(false)}
                                        className="p-3 hover:bg-sky-50 rounded-2xl transition-colors text-slate-400"
                                    >
                                        <X size={24} />
                                    </button>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-6 py-4 bg-white/50 border border-sky-100 rounded-2xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500/30 transition-all outline-none placeholder:text-slate-500 shadow-inner"
                                        ...
                                        <div className="grid grid-cols-3 gap-4">
                                            {['low', 'medium', 'high'].map((p) => (
                                                <button
                                                    ...
                                                    className={`py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${formData.priority === p ? 'bg-sky-500 border-sky-600 text-slate-800 shadow-lg shadow-sky-600/20' : 'bg-sky-50 border-sky-100 text-slate-400 hover:border-sky-500/30'}`}
                                                >
                                                    {p}
                                                </button>
                                            ))}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full px-6 py-4 bg-white/50 border border-sky-100 rounded-2xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500/30 transition-all outline-none placeholder:text-slate-500 resize-none shadow-inner"
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="flex-1 py-5 bg-sky-500 text-slate-800 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-xl shadow-sky-600/20 hover:bg-sky-700 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
                                    >
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setShowModal(false)}
                                        className="sm:w-1/3 py-5 bg-sky-50 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-sky-100 transition-all border border-sky-100"
                                    >
                                        Abort
                                    </button>
                                </div>
