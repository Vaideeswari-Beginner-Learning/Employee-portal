        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <AnimatePresence>
                {notification && (
                    ...
                    >
                        <div className={`px-8 py-4 rounded-2xl shadow-2xl border flex items-center gap-4 ${notification.type === 'error' ? 'bg-red-500/10 text-red-400 border-red-500/20' : notification.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-display font-black text-white tracking-tight leading-none uppercase">Field<span className="text-indigo-500 italic">.Documents</span></h1>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mt-3">Advanced Deployment Telemetry & Archive</p>
                </div>
                <button
                    onClick={downloadCSV}
                    className="btn-premium bg-indigo-600 text-white px-8 py-3.5 flex items-center gap-3 active:scale-95 transition-all shadow-lg shadow-indigo-600/20"
                >
                    <Download size={20} />
                ...
                <div className="p-8 border-b border-white/5 flex items-center gap-6 bg-white/5">
                    <div className="relative flex-1 w-full">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-900/50 border border-white/10 rounded-2xl py-3.5 pl-12 pr-6 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all text-white placeholder:text-slate-500 font-medium"
                        />
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filtered.length > 0 ? filtered.map((report) => (
                                <tr key={report._id} className="hover:bg-indigo-500/5 transition-all group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="text-[13px] font-black text-white leading-none mb-1.5 uppercase">{report.clientName}</p>
                                    ...
                                    <td className="px-8 py-6">
                                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest leading-none">{report.employee?.name || report.employeeName}</p>
                                    </td>
                                    <td className="px-8 py-6">
                                    ...
                                    <td className="px-8 py-6">
                                        <span className="text-[10px] font-black text-slate-400 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5 tabular-nums">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-3">
                                            <button
                                                ...
                                                className="px-4 py-2 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:bg-indigo-600 hover:text-white hover:border-indigo-600 hover:shadow-lg hover:shadow-indigo-600/20 transition-all flex items-center gap-2"
                                            >
                                                Inspect <ExternalLink size={14} />
                                            </button>
                                            <button
                                                onClick={(e) => handleDeleteReport(report._id, e)}
                                                className="p-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-500 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-all"
                ...
                {selectedReport && (
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-6 lg:p-12">
                        <motion.div
                            ...
                            className="bg-[#1E293B] rounded-[2.5rem] w-full max-w-6xl max-h-[90vh] overflow-hidden shadow-2xl border border-white/10 flex flex-col md:flex-row"
                        >
                            {/* Side Image / Visualization Section */}
                            <div className="md:w-[55%] bg-[#0F172A] flex items-center justify-center p-12 relative group border-r border-white/5">
                                ...
                                {selectedReport.image ? (
                                    ...
                                    <div className="text-center space-y-6 relative z-10">
                                        <div className="w-20 h-20 rounded-[2rem] bg-[#1E293B] flex items-center justify-center text-slate-700 mx-auto border border-white/5 shadow-sm">
                                            <Camera size={36} />
                                        </div>
                                        <p className="text-slate-600 text-[10px] font-black uppercase tracking-[0.5em]">No Visual Capture Detected</p>
                                    </div>
                                )}
                                <div className="absolute top-8 left-8 px-5 py-2.5 bg-indigo-500/10 rounded-2xl text-[10px] font-black text-indigo-400 uppercase tracking-[0.25em] border border-indigo-500/20 backdrop-blur-xl z-20 flex items-center gap-3 shadow-sm">
                            ...
                            {/* Deep Detail Section */}
                            <div className="md:w-[45%] p-12 lg:p-16 space-y-12 overflow-y-auto bg-[#1E293B]">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h2 className="text-3xl font-display font-black text-white tracking-tight leading-none mb-4 uppercase">{selectedReport.clientName}</h2>
                                        <div className="flex items-center gap-3 text-indigo-400">
                                            <MapPin size={14} className="animate-pulse" />
                                            <p className="text-[10px] font-black uppercase tracking-[0.2em]">{selectedReport.location}</p>
                                        </div>
                                    </div>
                                    <button onClick={() => setSelectedReport(null)} className="w-12 h-12 rounded-xl bg-white/5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all flex items-center justify-center group border border-white/5">
                                ...
                                <div className="grid grid-cols-2 gap-10 border-y border-white/5 py-10">
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-[13px] font-black text-white uppercase tracking-tight">{selectedReport.employee?.name || selectedReport.employeeName}</p>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] block">Hardware Matrix</label>
                                        <p className="text-[13px] font-black text-white uppercase tracking-tight tabular-nums">{selectedReport.cameraCount} Active Units</p>
                                    </div>
                                    <div className="space-y-2">
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="bg-[#0F172A] p-8 rounded-[1.5rem] border border-white/5 text-[13px] font-medium text-slate-400 leading-relaxed italic relative overflow-hidden group">
                                        <div className="absolute top-0 left-0 w-1.5 h-full bg-indigo-600/30" />
                                        "{selectedReport.issues || 'No anomalies detected during the physical deployment cycle and biometric verification.'}"
                                    </div>
                                </div>

                                <div className="pt-10 flex gap-6">
                                    <button className="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.25em] text-white bg-indigo-600 shadow-xl shadow-indigo-600/20 rounded-xl hover:bg-indigo-700 transition-all active:scale-[0.98]">Verify Artifact</button>
                                    <button
                                        onClick={(e) => handleDeleteReport(selectedReport._id, e)}
                                        className="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 bg-white/5 border border-white/10 rounded-xl hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-all active:scale-[0.98]"
                                    >
                                        Delete Record
                                    </button>
                                </div>
